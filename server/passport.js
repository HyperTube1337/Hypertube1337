    ...
    function (accessToken, refreshToken, profile, cb) {
      ...
      db.query("select user_id from users where user_id = ?", "Go" + profile.id, (err, res) => {
        if (err) res.send(err);
          ...
          db.query("SELECT COUNT(*) AS count FROM `users` WHERE `username` = ? OR `email` = ? LIMIT 1;", [profile._json.name, profile._json.email], (error, rslt) => {
            if (error) {
              // console.log(error);
              ...
              db.query("insert into users set firstname= ? , lastname = ? , username = ? , email = ? , user_id = ? , profilePic = ?, user_from = 'Google'", [
    ...
    function (accessToken, refreshToken, profile, cb) {
      db.query("select user_id from users where user_id = ?", "Gi" + profile.id, (err, res) => {
        if (err) res.send(err);
          ...
          db.query("SELECT COUNT(*) AS count FROM `users` WHERE `username` = ? OR `email` = ? LIMIT 1;", [profile._json.login, profile.emails[0].value], (error, rslt) => {
            if (error) {
              // console.log(error);
            } else if (rslt[0].count > 0) {
              // console.log("Email and or username are already used");
              ...
              db.query("insert into users set firstname= ? , lastname = ? , username = ? , email = ? , user_id = ? , profilePic = ?, user_from = 'Github'", [
                "",
                "",
                profile._json.login,
    ...
    function (accessToken, refreshToken, profile, cb) {
      db.query("select user_id from users where user_id = ?", "42" + profile.id, (err, res) => {
        if (err) res.send(err);
          ...
          db.query("SELECT COUNT(*) AS count FROM `users` WHERE `username` = ? OR `email` = ? LIMIT 1;", [profile._json.login, profile._json.email], (error, rslt) => {
            if (error) {
              // console.log(error);
            } else if (rslt[0].count > 0) {
              // console.log("Email and or username are already used");
              ...
              db.query("insert into users set firstname= ? , lastname = ? , username = ? , email = ? , user_id = ? , profilePic = ?, user_from = 'School'", [
