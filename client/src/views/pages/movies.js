	...
	function getMovieLink() {
		...
		axios
			...
			.then((response) => {
				...
				setTimeout(
					setVideoSrc({
						src: `http://localhost:3001/stream/${hash[0]}/${response.headers["content-type"]}`,
						type: "video/mp4",
					}),
					1000
				);
				setsubtitle({
					...
					en: {
						kind: "subtitles",
						src: `http://localhost:3001/stream/${hash[0]}/${movies.imdb_code}en.vtt`,
						srcLang: "en",
						default: true,
					},
					ar: {
						kind: "subtitles",
						src: `http://localhost:3001/stream/${hash[0]}/${movies.imdb_code}ar.vtt`,
						srcLang: "ar",
					},
					fr: {
						kind: "subtitles",
						src: `http://localhost:3001/stream/${hash[0]}/${movies.imdb_code}fr.vtt`,
						srcLang: "fr",
					},
				});
			});
